...
def tag_hosts(broken_cert_hosts):
                ...
                with ssl_context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    for interval in warn_intervals:
                        ...
                        if expiration_date < warn_date_time:
                            ...
                            #tag_hosts(cert_host, cert_status)
        ...
        if not source_q.empty():
            ...
            for port in cert_host.ports:
                ...
                if cert_status:
                    ...
                    cert_host.platform_host_url = ''.join( ['https://alpha.randori.io/' , str(short_name), '/hostnames/', str(cert_host.hostname_id) ] )
